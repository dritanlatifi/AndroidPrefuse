...
public class ActivityMap {
     ...
     * or the parent map.
     */
    public PActivity get(String key) {
        PActivity a = (PActivity)m_map.get(key);
     ...
     */
    public PActivity runAt(String key, long time) {
        PActivity a = get(key);
        if ( a != null )
     ...
     * @return the scheduled Activity, or null if not found
     */
    public PActivity run(String key) {
        PActivity a = get(key);
        if ( a != null )
     ...
     * keys are not found
     */
    public PActivity runAfter(String beforeKey, String afterKey) {
        PActivity before = get(beforeKey);
        PActivity after  = get(afterKey);
     ...
     * keys are not found
     */
    public PActivity alwaysRunAfter(String beforeKey, String afterKey) {
        PActivity before = get(beforeKey);
        PActivity after  = get(afterKey);
        if ( before != null && after != null )
     ...
     */
    public PActivity cancel(String key) {
        PActivity a = get(key);
        if ( a != null )
            a.cancel();
        return a;
     ...
     */
    public PActivity put(String key, PActivity activity) {
        return (PActivity)m_map.put(key, activity);
    }
