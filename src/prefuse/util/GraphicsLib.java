...
import awt.java.awt.geom.RectangularShape;
import awt.java.awt.geom.RoundRectangle2D;

import prefuse.render.AbstractShapeRenderer;
import prefuse.visual.VisualItem;

/**
 * Library of useful computer graphics routines such as geometry routines for computing the intersection of different shapes and rendering methods for computing bounds and performing optimized
 * drawing.
 * 
...
public class GraphicsLib
{
	...
	/**
	 * Computes the 2D convex hull of a set of points using Graham's scanning algorithm. The algorithm has been implemented as described in Cormen, Leiserson, and Rivest's Introduction to Algorithms.
	 * 
	...
	/**
	 * Compute a cardinal spline, a series of cubic Bezier splines smoothly connecting a set of points. Cardinal splines maintain C(1) continuity, ensuring the connected spline segments form a
	 * differentiable curve, ensuring at least a minimum level of smoothness.
	 * 
	 * @param pts
	 *            the points to interpolate with a cardinal spline
	 * @param slack
	 *            a parameter controlling the "tightness" of the spline to the control points, 0.10 is a typically suitable value
	 * @param closed
	 *            true if the cardinal spline should be closed (i.e. return to the starting point), false for an open curve
	 ...
	 * @param epsilon
	 *            threshold value under which to treat the difference between two values to be zero. Used to determine which segments to treat as lines rather than curves.
	 * @param slack
	 *            a parameter controlling the "tightness" of the spline to the control points, 0.10 is a typically suitable value
	 * @param closed
	 *            true if the spline should be closed (i.e. return to the starting point), false for an open curve
	 * @param tx
	 *            a value by which to translate the curve along the x-dimension
	 * @param ty
	 *            a value by which to translate the curve along the y-dimension
	...
	/**
	 * Sets a VisualItem's bounds based on its shape and stroke type. This method is optimized to avoid calling .getBounds2D where it can, thus avoiding object initialization and reducing object
	 * churn.
	 * 
	 * @param item
	 ...
	 * @param stroke
	 *            the stroke type that will be used for drawing the object, and may affect the final bounds. A null value indicates the default (line width = 1) stroke is used.
	 */
	...
	/**
	 * Render a shape associated with a VisualItem into a graphics context. This method uses the {@link java.awt.Graphics} interface methods when it can, as opposed to the {@link java.awt.Graphics2D}
	 * methods such as {@link java.awt.Graphics2D#draw(java.awt.Shape)} and {@link java.awt.Graphics2D#fill(java.awt.Shape)}, resulting in a significant performance increase on the Windows platform,
	 * particularly for rectangle and line drawing calls.
	 * 
	 * @param g
	 *            the graphics context to render to
	 * @param item
	 *            the item being represented by the shape, this instance is used to get the correct color values for the drawing
	 * @param shape
	 *            the shape to render
	 * @param stroke
	 *            the stroke type to use for drawing the object.
	 * @param type
	 *            the rendering type indicating if the shape should be drawn, filled, or both. One of {@link prefuse.render.AbstractShapeRenderer#RENDER_TYPE_DRAW},
	 *            {@link prefuse.render.AbstractShapeRenderer#RENDER_TYPE_FILL}, {@link prefuse.render.AbstractShapeRenderer#RENDER_TYPE_DRAW_AND_FILL}, or
	 *            {@link prefuse.render.AbstractShapeRenderer#RENDER_TYPE_NONE}.
	 */
	public static void paint(AndroidGraphics2D g, VisualItem item, Shape shape, BasicStroke stroke, int type)
	{
		...
		// TODO for Dritan: Currently drawShape is not working properly for "path" => antialiasing is not working.
		// therefore I deactivate sothat at least circles and rectangles are drawn correctly
		scale = 1;
		if (scale > 1.5)
		{
			if (fdraw)
